...
Linux sistem tepsisi için ayrıca `python3-xlib`, `python3-gi` ve Ayatana AppIndicator; ses için `pipewire-bin`, `pulseaudio-utils` veya `alsa-utils` paketlerinden biri gerekir. `.deb` paketinde ses biçimi dönüşümü sistemdeki `ffmpeg` ile yapılır. Kesin sürümler dağıtıma göre değişebilir.
